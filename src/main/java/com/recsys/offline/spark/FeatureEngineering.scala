...
object FeatureEngineering {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var movieResourcesPath : String = null
    var ratingsResourcesPath : String = null

    if (args.length < 2) {
        conf.setMaster("local")
          .setAppName("featureEngineering")
          .set("spark.submit.deployMode", "client")
      movieResourcesPath = this.getClass.getResource("/sampledata/movies.csv").getPath
      ratingsResourcesPath = this.getClass.getResource("/sampledata/ratings.csv").getPath
    } else {
        conf.setMaster("spark://master:7077")
          .setAppName("featureEngineering")
//          .set("spark.submit.deployMode", "client")
      movieResourcesPath = args(0)
      ratingsResourcesPath = args(1)
    }

    val spark = SparkSession.builder.config(conf).getOrCreate()
    val movieSamples = spark.read.format("csv").option("header", "true").load(movieResourcesPath)
    ...
    val ratingSamples = spark.read.format("csv").option("header", "true").load(ratingsResourcesPath)
