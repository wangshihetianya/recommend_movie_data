...
//      .withColumn("movieIdStr", array_join(col("movieIds"), " "))
      .withColumn("movieIdStr", concat_ws(" ", col("movieIds")))
...
//    val ratingsResourcesPath = this.getClass.getResource(rawSampleDataPath)
    ...
    if (embOutputFilename.contains("hdfs")) {
      val rdd = sparkSession.sparkContext.parallelize(userEmbeddings).map(row => Row(row._1 + "" + row._2.mkString(" ")))
      val structSchema: StructType = StructType(
        List(
          StructField("userEmb", StringType, true)
        )
      )
      val df = sparkSession.createDataFrame(rdd, structSchema)

      df.repartition(1).write.format("csv").option("header", "false").save(embOutputFilename)
    } else {
  ...
  def trainItem2vec(sparkSession: SparkSession, samples : RDD[Seq[String]], embLength:Int, embOutputFilename:String, saveToRedis:Boolean, redisKeyPrefix:String): Word2VecModel = {
    ...
    if (embOutputFilename.contains("hdfs")) {
      val rdd = sparkSession.sparkContext.parallelize(model.getVectors.toSeq).map(row => Row(row._1 + row._2.mkString(" ")))
      val structSchema: StructType = StructType(
        List(
          StructField("movieEmb", StringType, true)
        )
      )
      val df = sparkSession.createDataFrame(rdd, structSchema)
      df.repartition(1).write.format("csv").option("header", "false").save(embOutputFilename)
  ...
  def main(args: Array[String]): Unit = {
    ...
    var rawSampleDataPath : String = null
    var item2VecOutputFileName : String = null
    var graph2VecOutputFileName : String = null
    var userVecOutputFileName : String = null

    if (args.length < 4) {
      conf.setMaster("local")
        .setAppName("embedding")
        .set("spark.submit.deployMode", "client")
      rawSampleDataPath = this.getClass.getResource("/sampledata/ratings.csv").getPath
      item2VecOutputFileName = "item2vecEmb.csv"
      graph2VecOutputFileName = "itemGraphEmb.csv"
      userVecOutputFileName = "userEmb.csv"
    } else {
      conf.setMaster("spark://master:7077")
        .setAppName("embedding")
      rawSampleDataPath = args(0)
      item2VecOutputFileName = args(1)
      graph2VecOutputFileName = args(2)
      userVecOutputFileName = args(3)
    }
    ...
    val model = trainItem2vec(spark, samples, embLength, item2VecOutputFileName, saveToRedis = false, "i2vEmb")
    graphEmb(samples, spark, embLength, graph2VecOutputFileName, saveToRedis = false, "graphEmb")
    generateUserEmb(spark, rawSampleDataPath, model, embLength, userVecOutputFileName, saveToRedis = false, "uEmb")
  }
}
